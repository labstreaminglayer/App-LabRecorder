...
int main(int argc, char **argv) {
	try {
		require(argc == 2, "expected delayed, unavailable, or stalled");
		...
		// Read immediately, with no grace period after destruction: this is what a
		// CLI caller needs before exiting. A detached owner leaves this small file
		// in the writer's buffer.
		std::ifstream file(filename, std::ios::binary);
		const std::string contents((std::istreambuf_iterator<char>(file)), {});
		require(contents.substr(0, 4) == "XDF:",
				"writer was not flushed before destruction returned");
		require(contents.find("<sample_count>1</sample_count>") != std::string::npos,
